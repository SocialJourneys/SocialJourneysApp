	...
	private final  String USER_AGENT = "Mozilla/5.0";
	 ...
	 public void run() {
		 ...
		 DbConnect connectionObject = new DbConnect();
     	 try {
			pst = connectionObject.getDbConnect().prepareStatement("SELECT original_tweet_id,author,text,time_stamp FROM tweet order by time_stamp desc limit 10");
			rs = pst.executeQuery();
			...
			if (lastTweetResult.equals(currentTweetResult)) {
				...
				for (int i =0;i<temp.size();i++) {
					...
					Resource r = ResourceFactory.createResource("http://sj.abdn.ac.uk/ozStudyD2R/resource/ozstudy/twitter/tweet/"+ temp.get(i));
				...
				//	da.getModel().write(System.out);
					...
					if (answer) {
						...
						if(logger.isInfoEnabled()){
							logger.info(" \n Requesting annotations for tweet : http://sj.abdn.ac.uk/ozStudyD2R/resource/ozstudy/twitter/tweet/"+temp.get(i));
							
						}
						
						String rawData = "uri=http://sj.abdn.ac.uk/ozStudyD2R/resource/ozstudy/twitter/tweet/"+temp.get(i)+"&sparqEndpoint="+PredefinedConstants.REPOSITORY_SPARQL_ENDPOINT_URL+"&includeInference=on";
						//System.out.println(HttpRequests.sendPostRequest(rawData));
						String response = sendPostRequest (PredefinedConstants.ANNOTATION_TWEET_SERVICE_URL, rawData);
						//request annotate 
						...
						{
							//System.out.println("problem with the request for resource : http://sj.abdn.ac.uk/ozStudyD2R/resource/ozstudy/twitter/tweet/"+temp.get(i));
							
							if(logger.isInfoEnabled()){
								logger.info("Tweet annotation service probably not working. Check resource : http://sj.abdn.ac.uk/ozStudyD2R/resource/ozstudy/twitter/tweet/"+temp.get(i));
				...
				/*FileWriter fileWritter = new FileWriter("./Store/sjStore.ttl");
